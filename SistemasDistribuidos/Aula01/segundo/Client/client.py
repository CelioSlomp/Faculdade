...
SERVER_ADDR = ["http://191.52.6.159:5000"]
FILE_DIR = "./files"
...
class Client:
    ...
    def read_from_file(self, file_name):
        response = requests.get(f'{self.server_addr}/files/{file_name}').json()
...
def run():
    ...
    # -- -- -- -- adiciona na lista de execução (add_local, name_arquivo)
            ...
            client.list_server_files(verbose=True)
            for i in client.server_cur_list:
                if i not in 
