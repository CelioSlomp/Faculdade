function verificacoes(form) {
    /**Funçao para verificar as caixas do formulário de 
     * cadastro para ver se o usuário pode ser cadastrado.
     */
    const condicaoNome = form.nome.length >= 4 && form.nome.length <= 65;
    const condicaoEmail = form.email === form.emailConf;
    const condicaoSenha = (form.senha === form.senhaConf) && (form.senha.length > 8);
    const condicaoCpf = VerCPF(form.cpf);

    if (condicaoNome && condicaoEmail && condicaoSenha && condicaoCpf) {
        return true;
    }
    else if (!condicaoNome) {
        alert("Nome muito curto, tente outro.")
        return false;
    } else if (!condicaoEmail) {
        alert("Emails diferentes, tente novamente.")
        return false;
    } else if (!condicaoSenha) {
        alert("Senha diferente ou muito curta, tente novamente.")
        return false;
    } else if (!condicaoCpf) {
        alert("CPF Inválido.")
        return false;
    }
}

function VerCPF(cpf) {
    /**Função para verificar se o cpf que o usuario deseja
     * cadastrar é válido ou não. 
     */
    var soma;
    var resto;
    cpf = cpf.replace(/[^\d]+/g, '')
    soma = 0;
    if (!cpf || cpf.length != 11 || cpf == "00000000000" ||
        cpf == "11111111111" || cpf == "22222222222" || cpf == "33333333333" ||
        cpf == "44444444444" || cpf == "55555555555" || cpf == "66666666666" ||
        cpf == "77777777777" || cpf == "88888888888" || cpf == "99999999999") {
        return false;
    }
    for (i = 1; i <= 9; i++) {
        soma = soma + parseInt(cpf.substring(i - 1, i)) * (11 - i);
    }
    resto = (soma * 10) % 11;

    if ((resto == 10) || (resto == 11)) resto = 0;
    if (resto != parseInt(cpf.substring(9, 10))) {
        return false;
    }
    soma = 0;
    for (i = 1; i <= 10; i++) soma = soma + parseInt(cpf.substring(i - 1, i)) * (12 - i);
    resto = (soma * 10) % 11;

    if ((resto == 10) || (resto == 11)) {
        resto = 0;
    }

    if (resto != parseInt(cpf.substring(10, 11))) {
        return false;
    }
    return true;
}

/*
 * Jquery
 * Jquery
 * Jquery
 * Jquery
 * Jquery
*/

function irPara(pag) {
    window.location.href = pag;
}

$(document).ready(function () {
    $(document).on("click", "#botaoCadastro", function () {
        const nome = $("#nome").val();
        const cpf = $("#cpf").val();
        const email = $("#email").val();
        const emailConf = $("#emailConf").val();
        const senha = $("#senha").val();
        const senhaConf = $("#senhaConf").val();

        const submissao = {
            nome, cpf, email, emailConf, senha, senhaConf
        };

        if (verificacoes(submissao)) {
            alert("Cadastrado com sucesso :D")
            return true;
        }
    })
})